...
mat_size = 16
rng = default_rng(123)
A = rng.random(size=(mat_size, mat_size), dtype=np.float64)+1
B = rng.random(size=(mat_size, mat_size), dtype=np.float64)+1
# C = A @ B
...
# A.tofile('./src/sim/A.out', sep='\n', format='%016X')
# B.tofile('./src/sim/B.out', sep='\n', format='%016X')
...
def rand_data_gen(seed, M, N, K):
    ...
    A.tofile("./src/sim/build/A_{:d}_{:d}_{:d}.out".format(M, N, seed), sep="\n", format='%016X')
